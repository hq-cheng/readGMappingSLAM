 ...
 * 对于树的一个节点而言，无指向子节点的指针，只有指向parent的指针
 */
 ...
 * 起点，就是每个粒子（结构体中）对应的那个node，它没有孩子节点，只有父节点
 */
GridSlamProcessor::TNodeVector GridSlamProcessor::getTrajectories() const
{
    ...
    // 将每个粒子对应的那颗轨迹树中所有节点，置flag值为false
    ...
    // 将每个粒子（结构体中）对应的那个node存储到数组v中
    // 并将其父节点存储到队列border中
    // 并将(node->parent, node)暂存到parentCache中
    for (ParticleVector::const_iterator it=m_particles.begin(); it!=m_particles.end(); it++)
    {
	...
	// 返回存储有，每个粒子（结构体中）对应的那个node，的数组v
	return v;
}

/**
 * 集成所有的scan（这个函数没有被调用过。）
 */
...
/**
 * 把每个粒子对应的轨迹树中各个节点的 accWeight 清零
...
/**
 * 被下面的propagateWeights()函数调用，最终返回根节点的累计权重accWeight
 * 通过递归方法求解了一条路径树上根节点的累计权重accWeight
 * n，当前树的一个节点
 * weight，表示当前节点n的所有子节点的累计权重
 */
double propagateWeight( GridSlamProcessor::TNode* n, double weight ) {
    // 最终会递归到根节点（根节点没有父节点，直接返回根节点的累计权重accWeight）
    if ( !n ) {
        return weight;
    }
...
double GridSlamProcessor::propagateWeights() {
    ...
    // m_weights 数组中存储的是每一个粒子对应的轨迹树的叶子节点权重值
    std::vector<double>::iterator w = m_weights.begin();
    for ( ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++ ) {
        double weight = *w;
        // 所有粒子对应轨迹树的叶子节点，求它们的权重之和
        aw += weight;
